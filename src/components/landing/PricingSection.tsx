  ...
  return (
    <section className="bg-surface-grey py-20 pb-24 relative">
      <div className="container">
        ...
        <p className="text-center text-muted-foreground mb-12 text-lg">আপনার প্রয়োজন অনুযায়ী প্যাকেজ বেছে নিন</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto stagger-children">
            {packages?.map((pkg) => {
              ...
              return (
                <div
                  key={pkg.id}
                  className={`bg-background rounded-3xl p-7 shadow-card border-2 transition-all duration-300 hover:shadow-elevated hover:-translate-y-2 relative overflow-hidden ${style.border} ${style.bg}`}
                >
                  {style.badge && (
                    <div className="absolute -top-px left-1/2 -translate-x-1/2">
                      <span className="inline-flex items-center gap-1.5 gradient-hero-bg text-white text-xs font-bold px-5 py-2 rounded-b-2xl shadow-lg">
                  ...
                  <div className={style.badge ? "pt-6" : ""}>
                    ...
                    <p className="text-4xl font-extrabold gradient-text mt-4">
        ...
        {addons && addons.length > 0 && (
          <div className="max-w-4xl mx-auto mt-10 bg-background rounded-3xl p-7 shadow-card border border-border/40">
            <h3 className="font-bold text-foreground mb-4 text-lg">অতিরিক্ত সেবা (Add-ons):</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {addons.map((a) => (
                <div key={a.id} className="flex items-center gap-3 text-sm text-muted-foreground bg-muted/40 rounded-xl px-4 py-3">
        ...
        {/* Delivery policy */}
        {settings && (
          <div className="max-w-4xl mx-auto mt-6 bg-gradient-to-r from-primary/5 to-accent/5 rounded-3xl p-7 border border-primary/10">
